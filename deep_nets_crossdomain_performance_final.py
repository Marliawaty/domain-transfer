...
from candidate_models.model_commitments import brain_translated_pool, model_layer_def
...
def create_background_ids(hvm_data, rest_data, non_silhouette_data):
    ...
    for category in tqdm(CATEGORIES, desc='looping categories'):
        _logger.debug(f"Category round: {category}")
...
def get_decoder(data, estimator):
    ...
    #_logger.debug(f"fitting took {end - start:.2f} seconds")
...
#    num_neurons_arry = np.asarray((1, 3, 5, 10, 20, 30, 40, 50, 71))
    ...
    for split in np.arange(num_splits):
        ...
        for num_neurons, num_images_train in tqdm(itertools.product(num_neurons_arry, num_images_arry), desc='Neuron & image round'):
            _logger.debug(f"Current neuron & image round: {num_neurons, num_images_train, split}")
    ...
    _logger.debug(f"Dataframe took {end - start:.2f} seconds")
...
if __name__ == '__main__':
    ...
    estimator_ridge = RidgeClassifierCV(alphas=[0.0001, 0.001, 0.01, 0.1, 1, 10], fit_intercept=True, normalize=True)
    image_array = np.asarray((10, 20, 30, 40, 50, 60, 70, 80, 90))
    get_brain_model_performance(brain_model_name='alexnet', image_source='dicarlo.domain_transfer', estimator=estimator_ridge, image_arry=image_array,
                                penultimate_layer=False, split_num=SPLIT_NUMBER)

    ##### New models #######
#    brain_models_list = ['voneresnet-50-non_stochastic', 'resnet50-barlow',
#                         'custom_model_cv_18_dagger_408']

#    estimator_ridge = RidgeClassifierCV(alphas=[0.0001, 0.001, 0.01, 0.1, 1, 10], fit_intercept=True,
#                                        normalize=True)
#    image_array = np.asarray((10, 20, 30, 40, 50, 60, 70, 80, 90))

#    for brain_model in brain_models_list:
    #    for new_model_name in new_model_names:
#        get_brain_model_performance(brain_model_name=brain_model, image_source='dicarlo.domain_transfer', estimator=estimator_ridge,
#                                        image_arry=image_array, penultimate_layer=False, split_num=SPLIT_NUMBER)


    print('done')


    ########### test how to load new models #############
#    brain_model = get_brainmodel('ViT_L_32_imagenet1k', penultimate_layer=False)
...
#                                        normalize=True)
#    image_array = np.asarray((10, 20, 30, 40, 50, 60, 70, 80, 90))
#    get_brain_model_performance(brain_model_name='alexnet', image_source='dicarlo.domain_transfer', estimator=estimator_ridge, image_arry=image_array, penultimate_layer=False)
